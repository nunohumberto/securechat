...
def verifyUserCertificate(cert, cert_format):
	...
	auth_number = ""
	for c in cert_to_verify.get_issuer().get_components(): # What certificate do we need to get? Check the issuer field
		if c[0] == 'CN':
			auth_number = c[1][-4:]
			break

	ec_auth = readCertFromFile('certs/ecauth' + auth_number + '.cer', FILETYPE_ASN1) # Load the needed certificate

	auth_crls_to_add = []

	auth_counter = 1
	while True: 									
		try:	# Load CRLs that belong to the needed certificate only!
	...
	ec_cc_number = ""
	for c in ec_auth.get_issuer().get_components(): # Same thing as above
		if c[0] == 'CN':
			ec_cc_number = c[1][-3:]
			break
...
def verifySignature(cert_str, signature, data): # Verify signatures generated by clients
...
def verifySignature_pss(cert_str, signature, data): # Verify signatures generated by server
...
def sign_pss(key_filename, data): # Server loves this
